...
namespace DAL
{
    public class Activity
    {
        ...
        public IEnumerable<Model.Activity> GetVerifyCount(Model.Page page)
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
        ...
        public Model.Activity GetRecommed()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from activity where activityVerify='审核通过' and recommed='是' and endTime>=now() order by endTime desc limit 1";
